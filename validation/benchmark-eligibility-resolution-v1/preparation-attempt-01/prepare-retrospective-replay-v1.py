...
def main():
 ...
 for pdb in ('4MQS','5C1M','5JQH'):
  ...
  for seed in (1,2):
   ...
   # Historical reviewed role map is A=receptor/B=VHH for each of these targets.
   if target is None:target=dict(id=id,groupId=inventory[pdb]['relatednessGroup'],receptorChain='A',vhhChain='B',receptorSequence=proteins[0],vhhSequence=proteins[1]);targets.append(target)
   ...
   for i in range(5):
    ...
    if b is not None:
     expected=review[(job,i)];assert expected['requestSha256']==saved['sha256'];assert expected['modelChains']==dict(receptor='A',vhh='B')
