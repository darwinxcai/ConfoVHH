...
export const DOMAINS = [
  { accession: "PF00001", shortName: "7tm_1", description: "rhodopsin-family membrane domain", controlPdbId: "3SN6" },
...
export const QUERY_DEFINITIONS = [
  ...
  ...DOMAINS.map((domain) => ({ id: `control-${domain.accession.toLowerCase()}${domain.accession === "PF00001" ? "-corrected" : ""}`, kind: "domain-positive-control", domain: domain.accession, pdbId: domain.controlPdbId })),
  { id: "control-pf00001", kind: "domain-class-mismatch-check", domain: "PF00001", pdbId: "5UZ7", explanation: "Initial class-A control choice was incorrect: archived GPCRdb identifies 5UZ7 as calcitonin receptor, class B1. Preserve its attempted query; 3SN6 beta2AR is the corrected class-A control." },
];
...
export function buildRecentSearchQuery(definition, start = 0) {
  ...
  const query = !definition.kind.startsWith("recent-") ? group([term("rcsb_id", "exact_match", definition.pdbId), annotation]) : group([term("rcsb_accession_info.initial_release_date", "greater_or_equal", START), term("rcsb_accession_info.initial_release_date", "less", END), annotation]);
...
function schemaEvidence(records, raw) {
  ...
  try {
    ...
    // This external documentation schema includes valid escaped newlines in its
    // descriptions. It is not a signed study manifest; its exact bytes are archived.
    const schema = JSON.parse(raw.get(record.rawFile));
...
export function deriveRecentRcsbDiscovery({ source, records, raw }) {
  ...
  const discovery = searches.filter((row) => row.kind.startsWith("recent-"));
  ...
  for (const id of ids) {
    ...
    const attempts = [1, 2].map((repeat) => {
      try {
        ...
        assert(Object.keys(envelope).every((key) => ["data", "errors"].includes(key)), "Unexpected metadata envelope field");
        if (Object.hasOwn(envelope, "errors")) assert(Array.isArray(envelope.errors), "Invalid GraphQL errors field");
        ...
        assert.deepEqual(Object.keys(envelope.data), ["entries"], "Unexpected GraphQL data field");
...
export async function collectRecentRcsbDiscovery({ repositoryRoot = ROOT, outputDirectory = path.join(repositoryRoot, OUTPUT), fetchImpl = globalThis.fetch, now = () => new Date().toISOString() } = {}) {
  ...
  const ids = unique(replaySearches(records, raw).filter((row) => row.kind.startsWith("recent-")).flatMap((row) => row.observedIds));
...
export async function verifyRecentRcsbDiscovery({ repositoryRoot = ROOT, snapshotDirectory = path.join(repositoryRoot, OUTPUT) } = {}) {
  ...
  assert.equal(canonical(manifest.inputDigests), canonical(source.inputDigests));
  assert.equal(manifest.replayGeneratorScriptSha256 ?? manifest.generatorScriptSha256, sha(await readFile(HERE)));
  ...
  if (manifest.captureEpochs) {
    let nextRecord = 0;
    for (const epoch of manifest.captureEpochs) {
      assert.equal(epoch.firstRecordIndex, nextRecord);
      assert(epoch.lastRecordIndex >= epoch.firstRecordIndex);
      assert.equal(sha(await readFile(path.join(snapshotDirectory, epoch.scriptSourceFile))), epoch.scriptSha256);
      nextRecord = epoch.lastRecordIndex + 1;
    }
    assert.equal(nextRecord, records.length);
    assert.equal(manifest.captureEpochs[0].scriptSha256, manifest.generatorScriptSha256);
    const originalManifest = parse(await readFile(path.join(snapshotDirectory, "provenance/initial-manifest.json"), "utf8"));
    assert.equal(originalManifest.generatorScriptSha256, manifest.generatorScriptSha256);
    const initialRecords = parse(await readFile(path.join(snapshotDirectory, "provenance/initial-response-records.json"), "utf8"));
    assert.equal(canonical(initialRecords), canonical(records.slice(0, initialRecords.length)), "Original capture records changed");
  }
...
// Append the explicitly documented class-A control correction without relabeling
// earlier capture provenance. Raw earlier responses and request records stay intact.
export async function completeRecentRcsbControlCorrection({ repositoryRoot = ROOT, snapshotDirectory = path.join(repositoryRoot, OUTPUT), fetchImpl = globalThis.fetch, now = () => new Date().toISOString() } = {}) {
  const source = await inputFiles(repositoryRoot);
  const manifest = JSON.parse(await readFile(path.join(snapshotDirectory, "manifest.json"), "utf8"));
  assert(!manifest.captureEpochs, "Control correction has already been finalized");
  ...
  assert.equal(sha(await readFile(path.join(snapshotDirectory, "provenance/capture-generator-original.mjs"))), manifest.generatorScriptSha256, "Original capture source is not archived exactly");
  const raw = new Map(await Promise.all(records.map(async (record) => [record.rawFile, await readFile(path.join(snapshotDirectory, record.rawFile), "utf8")])));
  const script = await readFile(HERE); const scriptSha256 = sha(script);
  await writeFile(path.join(snapshotDirectory, "provenance/capture-generator-corrected.mjs"), script, { flag: "wx" });
  const definition = QUERY_DEFINITIONS.find((row) => row.id === "control-pf00001-corrected");
  assert(!records.some((record) => record.queryId === definition.id));
  assert.equal(source.gpcrdbMap.get("3SN6")?.protein, "adrb2_human");
  assert.equal(source.gpcrdbMap.get("5UZ7")?.protein, "calcr_human");
  for (const repeat of [1, 2]) await request({ output: snapshotDirectory, name: `${definition.id}-repeat-${repeat}-page-001`, endpoint: SEARCH, body: json(buildRecentSearchQuery(definition)), context: { kind: "search", queryId: definition.id, repeat, start: 0 }, records, raw, fetchImpl, now });
  await writeFile(path.join(snapshotDirectory, "query-definitions.json"), json(QUERY_DEFINITIONS));
  const built = deriveRecentRcsbDiscovery({ source, records, raw });
  for (const [name, content] of Object.entries(built.files)) await writeFile(path.join(snapshotDirectory, name), content);
  manifest.replayFinalizedAt = now(); manifest.replayGeneratorScriptSha256 = scriptSha256; manifest.responseRecordCount = records.length;
  manifest.captureEpochs = [
    { purpose: "Original discovery, metadata and control requests", firstRecordIndex: 0, lastRecordIndex: initialRecordCount - 1, scriptSourceFile: "provenance/capture-generator-original.mjs", scriptSha256: manifest.generatorScriptSha256 },
    { purpose: "Corrected 3SN6/PF00001 class-A control", firstRecordIndex: initialRecordCount, lastRecordIndex: records.length - 1, scriptSourceFile: "provenance/capture-generator-corrected.mjs", scriptSha256 },
  ];
  manifest.replayChanges = ["Preserve observed IDs from truncated pages while leaving completeness unresolved", "Parse ordinary external schema documentation with escaped newlines; retain strict parsing for study manifests and search/entry responses", "Exclude the original mismatched 5UZ7 control from discovery; execute and retain the corrected 3SN6 control"];
  await writeFile(path.join(snapshotDirectory, "manifest.json"), json(manifest));
  let readme = await readFile(path.join(snapshotDirectory, "README.md"), "utf8");
  readme = readme.replace(/Confirmed domain controls: \d+\/4/u, `Confirmed domain controls: ${built.summary.positiveControlsConfirmed}/4`).replace(/Specified query capture complete: (?:true|false)/u, `Specified query capture complete: ${built.summary.specifiedQueriesComplete}`);
  ...
  await writeFile(path.join(snapshotDirectory, "README.md"), readme);
  const names = (await filenames(snapshotDirectory)).filter((name) => name !== "checksums.sha256");
  await writeFile(path.join(snapshotDirectory, "checksums.sha256"), (await Promise.all(names.map(async (name) => `${sha(await readFile(path.join(snapshotDirectory, name)))}  ${name}`))).join("\n") + "\n");
  return built.summary;
}

if (process.argv[1] && path.resolve(process.argv[1]) === HERE) {
  const [command, directory] = process.argv.slice(2);
  const run = command === "collect" ? () => collectRecentRcsbDiscovery({ outputDirectory: directory ? path.resolve(directory) : path.join(ROOT, OUTPUT) }) : command === "verify" ? () => verifyRecentRcsbDiscovery({ snapshotDirectory: directory ? path.resolve(directory) : path.join(ROOT, OUTPUT) }) : command === "complete-control-correction" ? () => completeRecentRcsbControlCorrection({ snapshotDirectory: directory ? path.resolve(directory) : path.join(ROOT, OUTPUT) }) : null;
  if (!run) { console.error("Usage: node scripts/hard-decoy-v3/capture-recent-rcsb-discovery.mjs <collect|verify|complete-control-correction> [directory]"); process.exitCode = 1; }
