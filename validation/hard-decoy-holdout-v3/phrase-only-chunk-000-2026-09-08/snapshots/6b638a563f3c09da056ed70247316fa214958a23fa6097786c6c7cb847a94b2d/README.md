# Phrase-only discovery operational chunk 0
...
This immutable state snapshot is identified by all capture-record hashes. Verify the enclosing packet with `node scripts/hard-decoy-v3/capture-phrase-only-chunk.mjs verify --output-directory PACKET --chunk-index 0`. Restore global text metadata first with the existing restoration command.
