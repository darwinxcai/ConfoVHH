...
|---|---:|---|
| Unit and integration suite | [Commit-specific CI results](https://github.com/darwinxcai/ConfoVHH/actions/workflows/ci.yml) | Parsers, geometry calculations, exports, workers, provenance checks, and failure handling behave as specified |
...
| Cross-structure perturbation diagnostic | **1,222 retained poses from 17 public structures** | Tests behavior on native-derived perturbations; structures are not independent receptor/binder families |
...
Every decoy in that diagnostic was generated by rigidly perturbing a solved receptor–VHH complex. The twelve structures do not establish twelve independent receptor/binder families, and extending the structure panel does not establish absence of overfitting. This diagnostic does **not** establish performance on blind docking or on failure modes produced by real prediction pipelines, such as a plausible interface at the wrong epitope. An independent hard-decoy evaluation on prediction-derived poses remains necessary for an independent ranking-performance claim.

A [retrospective application to 165 retained GPCR prediction models](./validation/gpcr-paper-development-2026-09-04/README.md) now pairs unchanged ConfoVHH audits with official DockQ 2.1.3. Eight of fifteen `supported` geometry flags occur below DockQ 0.23. In the [within-job selection audit](./validation/gpcr-selection-development-2026-09-05/README.md), ConfoVHH selects an acceptable model in 18/33 jobs; the maximum exported predictor score does so in 19/33, averaging tied maxima equally. Only four jobs contain both acceptable and poor interfaces, all for 3P0G. These development results do not support native-pose selection superiority. Full PAE is unavailable for this cohort, and 85 other preliminary cognate models remain unavailable for rescoring.
