#!/usr/bin/env python3
"""Approved startup gates v2: unchanged frozen inference and restored GPU checks."""
import argparse
...
def remaining(value, current_epoch=None):
    ...
    require(0 < seconds <= MAX_SECONDS, 'Deadline must be within the remaining 3614.729-second cap')
    return seconds


def restored_gate(binding, base, base_sha):
    require(binding.get('schema') == 'confovhh.resumed-environment-binding.v1', 'Resume binding schema differs')
    require(binding.get('status') == 'ENVIRONMENT_VERIFIED_FOR_FRESH_GPU_CHECKS', 'Current environment verification required')
    require(binding.get('bundleSha256') == BUNDLE_SHA and binding.get('newBaseIdentitySha256') == base_sha, 'Current bundle/base binding differs')
    require(binding.get('environmentFilesVerified') is True, 'Current environment file verification required')
    require(base.get('verified') is True and base.get('verifiedBaseImageRef') == BASE, 'Fresh provider base identity required')
    root = Path('/workspace/executions') / binding['executionId'] / 'runtime-results'
    restore_path = root / binding['restoreReceiptPath']
    prior_base_path = root / binding['restoreBaseIdentityPath']
    require(sha(restore_path) == binding['restoreReceiptSha256'], 'Restore provenance differs')
    require(sha(prior_base_path) == binding['restoreBaseIdentitySha256'], 'Restore base provenance differs')
    restore, restore_base = read(restore_path), read(prior_base_path)
    require(restore.get('schema') == 'confovhh.same-base-runtime-restore.v1', 'Original restore schema differs')
    require(restore.get('status') == 'RESTORED_REQUIRES_ORIGINAL_RUNTIME_AND_FULL_GPU_JIT', 'Successful actual restore provenance required')
    require(restore.get('bundleSha256') == BUNDLE_SHA and restore.get('environmentVerifiedBeforeExecution') is True, 'Original restore identity differs')
    require(restore.get('baseIdentityReceiptSha256') == sha(prior_base_path), 'Original restore must bind its actual original base receipt')
    require(restore_base.get('verified') is True and restore_base.get('verifiedBaseImageRef') == BASE, 'Historical exact-base identity differs')
    require(binding.get('mode') in ('REUSED_PRESERVED_ENVIRONMENT', 'FRESH_RESTORE_FROM_SAME_BUNDLE'), 'Unknown environment mode')
...
def main():
    ...
    receipt = {'schema': 'confovhh.startup-gates.resume-v3', 'executionId': args.execution_id,
    ...
    try:
        ...
        paths = {'restore': results / 'environment-binding.json',
