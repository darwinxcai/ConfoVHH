...
import argparse, csv, gzip, hashlib, io, json, math, re, sys, time
import Bio
from Bio.PDB.MMCIF2Dict import MMCIF2Dict
from Bio.Data.PDBData import protein_letters_3to1_extended

parser = argparse.ArgumentParser(description=__doc__)
parser.add_argument("--root", type=Path, required=True, help="Recovered VERIFY directory")
parser.add_argument("--source", type=Path, required=True, help="Directory containing S2 and S5 source CSVs")
parser.add_argument("--manifest", type=Path, required=True, help="Previously reviewed cognate scoring_manifest.csv")
...
assert sha(MANIFEST) == REVIEWED_MANIFEST_SHA256, "Reviewed cognate manifest checksum mismatch"
plan = list(csv.DictReader(MANIFEST.open()))
cognate = {(j["job"], int(j["model"])) for j in plan}
...
writejson(OUT / "summary.json", {
    ...
    "priorAttempt": "../endpoint-recompute-prior-receipts/ records the initial overly broad alternate-conformer guard, correction, successful scratch run and executed scripts; endpoint definition and inclusion are unchanged.",
    "reviewedCognateManifestSha256": sha(MANIFEST),
