#!/bin/bash
# Additive startup wrapper; the frozen runner and its timeouts stay unchanged.
# The independent controller owns cloud shutdown and passes its actual deadline.
set -Eeuo pipefail
umask 077
if [[ $# != 2 || ! "$1" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]{0,127}$ ]]; then
  printf '%s\n' 'Usage: run-frozen-startup-v1.sh EXECUTION_ID ABSOLUTE_DEADLINE_UTC' >&2
  exit 2
fi
...
env -u PYTHONPATH -u PYTHONHOME /opt/confovhh-boltz/bin/python -I \
  "$helper_dir/verify-startup-gates-v1.py" "$1" "$2"
