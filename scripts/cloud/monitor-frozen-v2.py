#!/usr/bin/env python3
"""Versioned external monitor; the frozen scientific runner is never edited."""
...
def deadline_to_monotonic(value, utc_epoch=None, monotonic_now=None):
    ...
    if allowance <= 0 or allowance > 3284.221:
        raise ValueError('Deadline must be within the currently remaining 3284.221 seconds')
...
def main():
    ...
    receipt = {'schema': 'confovhh.external-frozen-monitor.v2', 'executionId': args.execution_id,
    ...
    try:
        with (results / 'controller-wrapper.stdout.log').open('xb') as out, (results / 'controller-wrapper.stderr.log').open('xb') as err:
            wrapper = subprocess.Popen(['/bin/bash', str(args.wrapper)], stdin=subprocess.DEVNULL,
