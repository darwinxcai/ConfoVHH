...
const SOURCE_CHECKSUM = "796aa0734da86791f8ccdef92c7de03da22bdafd7818a5c5e95d73b6b5d35ebc";
const LIMIT = 16 * 1024 * 1024;
const ROUTE = "C.2_GPCRDB_FROZEN_INVENTORY_COMPLEMENT";
...
async function context(repositoryRoot) {
  ...
  const checksums = await input(`${SOURCE}/checksums.sha256`);
  ok(sha(checksums) === SOURCE_CHECKSUM, "Frozen source checksum manifest changed.");
  const checksumRows = checksums.toString("utf8").trimEnd().split("\n").map((line) => /^([a-f0-9]{64})  ([A-Za-z0-9._/-]+)$/u.exec(line));
  ok(checksumRows.every(Boolean), "Invalid frozen source checksums.");
  async function frozen(relative) {
    const matches = checksumRows.filter((row) => row[2] === relative);
    ok(matches.length === 1, `Frozen source is not bound exactly once: ${relative}`);
    const bytes = await input(`${SOURCE}/${relative}`);
    ok(sha(bytes) === matches[0][1], `Frozen source digest mismatch: ${relative}`);
    return bytes;
  }
  const allIds = identifiers(await frozen("normalized/gpcrdb-api.txt"), "GPCRdb IDs");
  const historicalIds = identifiers(await frozen("normalized/rcsb-gpcrdb-intersection.txt"), "Historical IDs");
  ok(allIds.length === 1716 && historicalIds.length === 287, "Frozen GPCRdb/historical counts changed.");
  ok(historicalIds.every((id) => allIds.includes(id)), "Historical intersection is not a GPCRdb subset.");
  const historical = new Set(historicalIds);
  const ids = allIds.filter((id) => !historical.has(id));
  ok(ids.length === 1429, "GPCRdb complement must contain exactly 1,429 IDs.");
  const gpcrdbRows = parse(await frozen("raw/gpcrdb-api-1.json"));
  const gpcrdbMap = new Map();
  for (const row of gpcrdbRows) {
    const id = String(row.pdb_code ?? "").toUpperCase();
    if (!allIds.includes(id)) continue;
    ok(!gpcrdbMap.has(id), `Duplicate frozen GPCRdb entry: ${id}`);
    gpcrdbMap.set(id, row);
  }
  ok(allIds.every((id) => gpcrdbMap.has(id)), "GPCRdb metadata does not cover its frozen inventory.");
  const contract = parse(await input(CONTRACT));
  const queryBytes = await input(contract.rcsb.queryFile);
  ok(sha(queryBytes) === contract.rcsb.querySha256, "Pinned metadata GraphQL query changed.");
  ...
  await input(NORMALIZER);
  await input("scripts/hard-decoy/oracle/canonical-json.mjs");
  inputs.set("scripts/hard-decoy-v3/gpcrdb-complement-metadata.mjs", sha(await direct(HERE)));
  ...
  const plan = {
    ...
    frozenInventoryDate: "2026-08-29", frozenGpcrdbEntryCount: allIds.length, historicalIntersectionEntryCount: historicalIds.length,
    complementEntryCount: ids.length, complementIdentifiersSha256: sha(`${ids.join("\n")}\n`),
...
async function checkedOutput(directory, root, create) {
  ...
  ok(output !== root && output !== validation && !output.startsWith(path.join(validation, "source-snapshot-2026-08-29"))
    && !output.startsWith(path.join(validation, "entry-metadata-snapshot-2026-08-29")), "Use a new, isolated complement output directory.");
...
export async function prepareGpcrdbComplementMetadata({ repositoryRoot = ROOT, outputDirectory }) {
  ...
  return { status: "GPCRDB_COMPLEMENT_PREPARED", requestedEntries: ctx.ids.length, requests: ctx.plan.requests.length, outputDirectory: directory };
}
...
async function retrieve(ctx, directory, request, fetchImpl, now, delay) {
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    const failureRaw = `failures/${stem}-attempt-${attempt}-body.json`;
    const failureRecord = `failures/${stem}-attempt-${attempt}-capture.json`;
...
async function failedCaptures(ctx, directory) {
  ...
  for (const name of names.filter((item) => item.endsWith("-capture.json")).sort()) {
    const match = /^(batch-\d{3}-repeat-[12])-attempt-([123])-capture\.json$/u.exec(name);
...
async function derived(ctx, directory) {
  ...
  const summary = { schemaVersion: "1.0.0", studyId: ctx.plan.studyId, discoveryRoute: ROUTE,
    status: missing.length === 0 ? "GPCRDB_COMPLEMENT_METADATA_CAPTURED_PENDING_SCIENTIFIC_REVIEW" : "GPCRDB_COMPLEMENT_METADATA_INCOMPLETE_MISSING_ENTRIES",
    frozenGpcrdbEntryCount: 1716, historicalIntersectionEntryCount: 287, requestedEntryCount: ctx.ids.length,
    ...
    discoveryRouteMetadataCaptureComplete: missing.length === 0, routeC2ScientificDispositionComplete: false,
    broaderDiscoveryComplete: false, frozenInventoryDate: "2026-08-29", inventoryFreshnessClaimed: false,
  ...
  const files = new Map([
    ...
    ["summary.md", `# GPCRdb inventory complement metadata capture\n\nThe frozen 2026-08-29 GPCRdb inventory contains 1,716 entries. Removing the 287-entry historical keyword intersection leaves exactly 1,429 entries. All 1,429 are requested afresh in 58 batches, each repeated twice.\n\nCaptured: ${entries.length}; missing in both repeats: ${missing.length}; raw responses: ${captures.length}. Metadata signals do not establish VHH identity, direct receptor binding, exclusions, or independent components. Entries lacking a VHH descriptor remain pending and require sequence and source review.\n\nThis captures the complement of one frozen inventory. It does not claim an up-to-date or exhaustive GPCR census, completion of every discovery route, or completion of C.2 scientific review. The broader census remains incomplete and the protocol remains DRAFT/BLOCKED. No target freeze, native-coordinate access, label access, model execution, or performance claim is authorized by this package.\n`],
  ]);
...
export async function collectGpcrdbComplementMetadata({ repositoryRoot = ROOT, outputDirectory, fetchImpl = fetch, now = () => new Date().toISOString(), delay = (milliseconds) => new Promise((resolve) => setTimeout(resolve, milliseconds)), onProgress = () => {} }) {
  await prepareGpcrdbComplementMetadata({ repositoryRoot, outputDirectory });
  const ctx = await context(repositoryRoot);
  ...
  return await verifyGpcrdbComplementMetadata({ repositoryRoot, snapshotDirectory: directory });
}
export async function verifyGpcrdbComplementMetadata({ repositoryRoot = ROOT, snapshotDirectory }) {
  const ctx = await context(repositoryRoot);
...
if (process.argv[1] && path.resolve(process.argv[1]) === HERE) {
  ...
  try {
    ok(["prepare", "collect", "verify"].includes(command) && directory, "Usage: gpcrdb-complement-metadata.mjs prepare|collect|verify <directory> [repository-root]");
    ...
    const result = await (command === "prepare" ? prepareGpcrdbComplementMetadata(options) : command === "collect" ? collectGpcrdbComplementMetadata(options) : verifyGpcrdbComplementMetadata(options));
