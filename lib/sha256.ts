...
function compress(state: Uint32Array, block: Uint8Array, offset: number, words: Uint32Array): void {
...
export function sha256HexSoftware(input: ArrayBuffer): string {
  ...
  for (let offset = 0; offset < completeBytes; offset += 64) {
    compress(state, bytes, offset, words);
  }

  const remainder = bytes.byteLength - completeBytes;
  const finalLength = remainder < 56 ? 64 : 128;
  const finalBlocks = new Uint8Array(finalLength);
  finalBlocks.set(bytes.subarray(completeBytes));
  ...
  lengthView.setUint32(finalLength - 8, bitLengthHigh, false);
  lengthView.setUint32(finalLength - 4, bitLengthLow, false);
  for (let offset = 0; offset < finalLength; offset += 64) {
    compress(state, finalBlocks, offset, words);
  }
...
export async function sha256Hex(input: ArrayBuffer): Promise<string> {
  ...
  return sha256HexSoftware(input);
}
